...
webrdirect="127.0.0.1/library/webr-direct"
webrproxy="127.0.0.1/library/webr-proxy"
...
docker build -t $webrdirect --build-arg type=direct --build-arg rport=3582 --build-arg rws=3592 .
...
docker login -u tvu -p P1dkme17tvU 127.0.0.1
docker push $webrdirect
docker push $webrproxy
echo "======push image to harbor end======"

echo "======start containers======"
docker run -d --name 23.66 -p 3366:80 -p 4366:443 -e RHOST=10.12.23.66 127.0.0.1/library/webr-direct
docker run -d --name 23.20 -p 3320:80 -p 4320:443 -e RHOST=10.12.23.20 127.0.0.1/library/webr-proxy
docker run -d --name 23.21 -p 3321:80 -p 4321:443 -e RHOST=10.12.23.21 127.0.0.1/library/webr-proxy
docker run -d --name 32.91 -p 3391:80 -p 4391:443 -e RHOST=10.12.32.91 127.0.0.1/library/webr-proxy
